#include "algorithm_feedback_adapter/example_battery_feedback_adapter.hpp"
...
ExampleBatteryFeedbackAdapter::ExampleBatteryFeedbackAdapter(const rclcpp::NodeOptions & options)
: Node("algorithm_feedback_adapter_cpp_node", options)
{
  ...
  sub_ = this->create_subscription<InputMsg>(
    ...
    std::bind(&ExampleBatteryFeedbackAdapter::on_msg, this, std::placeholders::_1));
}

std::vector<MetricSample> ExampleBatteryFeedbackAdapter::extract_metrics(const InputMsg & msg)
{
...
void ExampleBatteryFeedbackAdapter::on_msg(const InputMsg::SharedPtr msg)
{
