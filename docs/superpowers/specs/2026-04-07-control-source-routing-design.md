...
不新增自定义 service，直接复用 ROS 2 节点自带的参数服务：

- `/safety_emergency_executor/set_parameters`

外部通过更新字符串参数 `active_control_source` 完成模式切换。

语义：

- 参数值必须是 4 个 canonical source 之一
- 合法时：
  - 更新 `active_source`
  - 发布状态 topic
  - 参数服务返回成功
...
- 外部若只需要当前模式，可优先订阅状态 topic；若需要主动查询，则读取该参数
...
3. 创建 4 路来源订阅
4. 注册 `active_control_source` 参数校验回调，并准备状态 topic
...
外部通过 `/safety_emergency_executor/set_parameters` 更新 `active_control_source` 时：
...
本期不新增任何自定义 `srv/msg` 接口。

设置与查询统一复用 ROS 2 标准参数服务：

- 设置：`/safety_emergency_executor/set_parameters`
- 查询：`/safety_emergency_executor/get_parameters`

这意味着构建层不需要引入 `rosidl_generate_interfaces(...)`，
只需要在节点中增加参数声明、参数回调和测试侧参数客户端使用即可。
...
- `/set_manual_override` -> `/safety_emergency_executor/set_parameters(active_control_source=...)`
- `/get_manual_override` -> `/safety_emergency_executor/get_parameters(active_control_source)`
...
- 合法来源切换成功
- 非法来源切换失败且保持原值
- 当前来源查询值可直接读取为 `active_control_source`
...
- 参数服务修改 `active_control_source` 后，节点立即切换来源
...
- 默认参数文件包含 `active_control_source`
