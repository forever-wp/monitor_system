...
- 说明：`v2.4.1` 的功能补充版本
- 增加内容：
  - 新增 `acc_update_topic`，支持通过 topic 直接覆盖当前加速度值
  - 保留默认 `acc_` 作为未覆盖时的兜底值
  - 完善 `nav2_monitor` 与 `safety_emergency_executor` 当前生效配置参数注释
- 使用建议：如果你在 `v2.4.1` 基础上还需要在线修改加速度值，优先使用该版本
